...
export default class Manger extends Component<Config['manger']> {
  ...
  private manger() {
    ...
    if (this.config.stopCmdEnabled) btnList.push({ id: 'stop', text: '关闭服务器', action: 'stop' });
    ...
    mangerCmd.setCallback((_, { player: pl }, out, { action }) => {
      ...
      switch (action) {
        ...
        case 'crash':
          Gui.send(pl, {
            title: '强制崩玩家',
            buttons: mc.getOnlinePlayers().map((pl) => ({ text: pl.realName, action: `crash "${pl.realName}"` }))
          });
          break;
        case 'info':
          Gui.send(pl, {
        ...
        case 'ban':
          Gui.send(pl, {
            ...
            elements: [
              ...
              { title: '封禁IP', type: 'switch', default: false },
  ...
  private vanish() {
    ...
    vanishCmd.setCallback((_, { player: pl }, out) => {
      ...
      pl.addEffect(14, Infinity, 225, false);
      out.success('已设置隐身状态');
    });
  }
  ...
  private ban() {
    function banKick(pl: Player) {
      ...
      if (ip in allBans && allBans[ip].time < Date.now()) {
        delete allBans[ip];
        return;
      }
      if (name in allBans && allBans[name].time < Date.now()) {
        delete allBans[name];
        return;
      }
      ...
      pl.kick(
        `${ip in allBans ? `当前 IP（${ip}）` : '你'} }已被服务器封禁${time ? `到 ${time.toLocaleString()}` : '永久'}${reason ? `\n原因：${reason}` : ''}`
      );
    ...
    banCmd.setCallback((_, __, out, result) => {
      ...
      allBans[result.action === 'banip' ? targetPl.getDevice().ip : player.toLocaleString()] = {
    ...
    mc.listen('onJoin', (pl) => pl && banKick(pl));
  }
    ...
    mc.listen('onJoin', (pl) => {
      network.httpGet(
        ...
        (status, result) => {
          ...
          if (JSON.parse(result).status !== 2001) return;
  ...
  private crash() {
    const crashCmd = this.cmd('crash', '强制崩玩家客户端', PermType.GameMasters);
    crashCmd.mandatory('player', ParamType.String);
    crashCmd.overload(['player']);
    crashCmd.setCallback((_, __, ___, { player }) => mc.getPlayer(player)?.crash());
  }

  private stop() {
    const stopCmd = this.cmd('stop', '关闭服务器', PermType.GameMasters);
